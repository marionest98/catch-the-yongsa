...
public class GridManager : MonoBehaviour
{
    ...
    // 되돌리기용 이전 grid 상태 저장 변수
    private CellType[,] prevGrid;
    ...
    // 현재 grid 상태 저장 - 성물 배치 확정 직전에 호출
    public void SaveState()
    {
        StageData stageData = StageManager.Instance.stageData;
        prevGrid = new CellType[stageData.width, stageData.height];
        for (int x = 0; x < stageData.width; x++)
        {
            for (int y = 0; y < stageData.height; y++)
            {
    ...
    // 저장된 grid 상태로 복구 - 되돌리기 버튼 눌렀을 때 호출
    public void RestoreState()
    {
        StageData stageData = StageManager.Instance.stageData;

        // 현재 배치된 성물 오브젝트 전부 제거
        foreach (GameObject obj in relicObjects.Values)
        {
            Destroy(obj);
        }
        relicObjects.Clear();
